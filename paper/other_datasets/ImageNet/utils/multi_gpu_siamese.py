# -*- coding: utf-8 -*-
"""The default implementation to run a keras model using multiple GPUS does not
handle siamese models with multiple inputs appropriately. This module
contains and alternative version of the default function.

This alternative model was used during the experiment with CBCs on ImageNet.
"""
...
def multi_gpu_siamese(model, gpus=None, cpu_merge=True, cpu_relocation=False):
    ...
    for i, gpu_id in enumerate(target_gpu_ids):
        with tf.device('/gpu:%d' % gpu_id):
            with tf.name_scope('replica_%d' % gpu_id):
                ...
                outputs = model(inputs) # Also append the non feed inputs
